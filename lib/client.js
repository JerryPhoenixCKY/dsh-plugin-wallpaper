...
function initialCrop(natW, natH, targetAspect) {
...
function resizeCrop(start, mode, handle, dx, dy, aspect) {
  ...
  if (aspect === null) {
    ...
    if (left) {
      w2 = start.w - dx;
      x2 = start.x + start.w - w2;
    }
    if (right) w2 = start.w + dx;
    if (top) {
      h2 = start.h - dy;
      y2 = start.y + start.h - h2;
    }
    if (bottom) h2 = start.h + dy;
    w2 = clamp(w2, MIN_CROP, 1);
    h2 = clamp(h2, MIN_CROP, 1);
    x2 = clamp(x2, 0, 1 - w2);
    y2 = clamp(y2, 0, 1 - h2);
...
function attachBackground(ctx, store) {
  ...
  const paint = () => {
    ...
    const tokens = theme.active?.tokens ?? {};
